...
/* ---------- store di-inject: dev = JSON (store-json.mjs), produksi (Vercel) =
 * Supabase (store-supabase.mjs). Default JSON agar modul tetap berfungsi bila
 * dipakai tanpa pasangStore(). ---------- */
let storeApi = null
export function pasangStore(api) { storeApi = api }
let sseAktif = true
export function aturSseAktif(v) { sseAktif = !!v }
...
/* ---------- store: { orders, pengaturan, pelanggan, sesi, subs, vapid, webhook } ---------- */
/* Antarmuka store ({ muatStore, simpanStore }) di-inject via pasangStore().
 * simpanStore BOLEH async (Supabase) — karena itu setiap pemanggilan di dalam
 * penanganOrder memakai `await` (aman untuk store JSON yang sinkron juga). */
const __storeDefault = buatStoreJsonOrder(process.env.ORDERS_FILE || join(__dirname, 'data', 'orders.json'))
function muatStore() {
...
async function pastikanVapid(store) {
...
async function konfigPush(store) {
  await pastikanVapid(store)
...
/* Penangan permintaan order — dipakai server dev (node:http) maupun Vercel
 * (req/res standar Node http). SSE hanya aktif di jalur dev (sseAktif); di
 * serverless endpoint /api/events mengembalikan 501 (pakai Realtime, P3). */
export async function penanganOrder(req, res) {
  ...
  if (url.pathname === '/api/events' && req.method === 'GET') {
    if (!sseAktif) {
      return json(res, 501, { ok: false, alasan: 'SSE tidak tersedia di hosting serverless — gunakan Supabase Realtime broadcast.' })
    }
  ...
  if (m) {
    ...
    if (m[2] === 'cek-pembayaran' && req.method === 'POST') {
      ...
      if (h.lunas) await simpanStore(store)
...
/* cekGatewayPembayaran diekspor untuk polling gateway otomatis di jalur dev
 * (server/order.mjs) — serverless tidak punya setInterval, jadi jalur produksi
 * mengandalkan webhook QRIS Bridge + polling per-pesanan oleh klien. */
export { cekGatewayPembayaran }
